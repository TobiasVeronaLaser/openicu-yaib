...
def validate_dynamic_columns(
    df: pl.LazyFrame, dynamic_vars: list[str] | None = None
) -> dict[str, list[str]]:
...
def ricu_range_report(
    df: pl.LazyFrame, ricu_meta: RicuConceptMeta, dynamic_vars: list[str] | None = None
) -> pl.DataFrame:
    vars_ = dynamic_vars or DYNAMIC_VARS
    actual_cols = set(df.collect_schema().names())
        ...
        rows.append(
            df.select(
                [
                    ...
                    pl.lit(None if lower is None else float(lower))
                    .cast(pl.Float64)
                    .alias("ricu_min"),
                    pl.lit(None if upper is None else float(upper))
                    .cast(pl.Float64)
                    .alias("ricu_max"),
                ]
            ).collect()
    ...
    return (
        ...
        .with_columns(
            [
                (
                    pl.col("ricu_min").is_not_null()
                    & pl.col("observed_min").is_not_null()
                    & (pl.col("observed_min") < pl.col("ricu_min"))
                ).alias("below_ricu_min"),
                (
                    pl.col("ricu_max").is_not_null()
                    & pl.col("observed_max").is_not_null()
                    & (pl.col("observed_max") > pl.col("ricu_max"))
                ).alias("above_ricu_max"),
            ]
        )
        .sort("concept")
    )
...
def stay_windows_from_dyn(df: pl.LazyFrame) -> pl.DataFrame:
    ...
    return (
        ...
        .with_columns(
            (pl.col("dyn_n_timepoints") - pl.col("expected_n_timepoints")).alias(
                "missing_grid_points"
            )
        )
        .sort("stay_id")
...
def compare_dyn_to_icustays(
    ...
    *,
    ...
    stay_windows = (
        stays.with_columns(
            (
                (pl.col("outtime") - pl.col("intime")).dt.total_seconds().cast(pl.Float64) / 3600.0
            ).alias("los_hours")
        )
    ...
    dyn_windows = dyn.group_by("stay_id").agg(
        [
            pl.col("time").min().alias("dyn_start"),
            pl.col("time").max().alias("dyn_end"),
            pl.col("time").n_unique().alias("dyn_n_timepoints"),
...
def debug_concept_against_output(
    *,
    ...
    comparison = manual.join(
        final, on=["stay_id", "time"], how="inner", suffix="_final"
    ).with_columns((pl.col(concept) - pl.col(f"{concept}_final")).abs().alias("abs_diff"))
